...
\end{verbatim}
to \verb|~/.Rprofile|, then if any of these five functions are used,
...
With these autoloads in place we can add further lines to
\verb|~/.Rprofile| such as:
\begin{verbatim}
SOAR::AttachUtils()
\end{verbatim}
which will ensure that the central utilities cache is part of the
search path at startup. We need the double colon construction here to
let the interpreter know where to fine the function
\texttt{AttachUtils}.  However these variants on the main functions
with \texttt{Data} or \texttt{Utils} tacked on to the end of their
names are front-ends to the basic functions, and actually morph into a
call to the relevant basic function.  In turn, the basic function
causes the package to be loaded through the autoload.  So a
side-effect will be that \texttt{SOAR} will be loaded on the search
path at startup as well, just before the \verb|.R_Utils| cache.
...
@ %def
Now using \texttt{any} (exported) function from \texttt{SOAR} would
